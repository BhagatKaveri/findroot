...
findroot::findroot(double x,double y)
{
    a=x;
    b=y;
}
...
void findroot  :: get()
{
    ...
    cout<<"Enter the initial guesses:\na=";    //Enter the value of a for later use with goto)
    cin>>a;
...
double findroot :: root(double a,double b)
{
    ...
    {                //If f(a)*f(b)>0 then the root does not exist between a and b
        ...
        cout<<"incorrect roots chosen\n" ;           //go back to 'a' ie 17 and ask for different values of a and b
   
   //return;
   }
    else                //else a root exists between a and b
    {
    while (fabs(a-b)>=error())        /*if the mod of a-b is greater than the accuracy desired keep                         bisecting the interval*/
    {
